...
struct PadEditRecordTests {
    ...
    func oneTapPerKey() async {
        ...
        // Started, not awaited. Awaiting here would hang rather than fail when
        // the guard is missing — the second call reaches the held write and
        // sits there. That is the third time in this suite's history, so it is
        // written the safe way from the start.
        let second = Task { await edit.remove(7) }
        store.holds = false
        _ = await first.value

        #expect(await second.value == nil, "a second tap was accepted")
