...
- **Documentation rule:** every top-level type and protocol carries a one-line `///` saying
  what it is for; private and nested implementation does not, unless something non-obvious
  warrants a *why* comment. `.coderabbit.yaml` holds the matching threshold — change them
