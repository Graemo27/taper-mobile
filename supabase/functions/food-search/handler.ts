...
/**
 * The whole request path, as a plain function.
 *
 * Separated from `index.ts` so it can be called directly by a test. Importing a
 * module that calls `Deno.serve` at top level starts a server, which is why the
 * entrypoint is one line elsewhere rather than a flag checked here — a
 * conditional would change what the deployed runtime does, and this does not.
 */
